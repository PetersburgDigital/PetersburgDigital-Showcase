   ...
   hero.addEventListener('mouseover', () => {
    function sleep(ms) {
      ...
      sleep(3000).then(() => { 
        ...
        blockImg.style.backgroundImage = 'url(PetersburgDigital-Showcase/img/kuchenland.png)'
     });
     sleep(7000).then(() => { 
        listElements[1].classList.remove('child--active')
        listElements[2].classList.add('child--active')
        blockImg.style.backgroundImage = 'url(PetersburgDigital-Showcase/img/ralph-lauren.png)'
     });
     sleep(11000).then(() => { 
        listElements[2].classList.remove('child--active')
        listElements[3].classList.add('child--active')
        blockImg.style.backgroundImage = 'url(PetersburgDigital-Showcase/img/easyDrive.png)'
     });
     sleep(15000).then(() => { 
        listElements[3].classList.remove('child--active')
        listElements[4].classList.add('child--active')
        blockImg.style.backgroundImage = 'url(PetersburgDigital-Showcase/img/movie-night.png)'
     });
   })
    ...
    click.addEventListener('mouseover', () => {
        if (click === listElements[0]) {
            blockImg.style.backgroundImage = 'url(PetersburgDigital-Showcase/img/real-estate.jpeg)'
        }
